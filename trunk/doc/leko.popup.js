...
b=v.enabled,f=$(v.target),s=Math.round,a=v.shadow,w,h,m,n,x,y,p;if(f.length&&f.visible()){m=o.css({position:"absolute"}).transparent(1).insertAfter(f).outerWidth();n=o.outerHeight();w=f.outerWidth();h=f.outerHeight();p=f.position();o.css({left:p.left+[-m,0,s(w/2-m),s((w-m)/2),s(w/2),w-m,w][v.posX||0]+(v.offsetX||0)+"px",top:p.top+[-n,0,s(h/2-n),s((h-n)/2),s(h/2),h-n,h][v.posY||0]+(v.offsetY||0)+"px"});if(a)o.shadow(a);o.transparent();}},{shadow:{}},"shadow",function(o,v,d,c){var
...
delete d._;d._=function(){g=n&&o.visible(1);if(g){p=o[c]();e.height(o.outerHeight()).width(o.outerWidth()).css({top:p.top+y-r+(y>0?-1:y<0?1:0)+"px",left:p.left+x-r+(x>0?-1:x<0?1:0)+"px",zIndex:o.css("zIndex")}).insertBefore(o);}
