...
f=$(v.target),s=Math.round,a=v.shadow,w,h,m,n,x,y,p;if(f.length){o.css({position:"absolute"}).insertAfter(f).show();w=f.outerWidth();h=f.outerHeight();m=o.outerWidth();n=o.outerHeight();p=f.position();o.css({left:p.left+[-m,0,s(w/2-m),s((w-m)/2),s(w/2),w-m,w][v.posX||0]+(v.offsetX||0)+"px",top:p.top+[-n,0,s(h/2-n),s((h-n)/2),s(h/2),h-n,h][v.posY||0]+(v.offsetY||0)+"px"});if(a)o.shadow(a);console.log(o.data("shadow").$);}},{},"shadow",function(o,v,d,c){var
