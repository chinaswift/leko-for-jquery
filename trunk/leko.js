(function(window,document,$){
	...
	var
		...
		_={
			...
			colors:{
				properties:['color','backgroundColor','borderColor','borderBottomColor','borderLeftColor','borderRightColor','borderTopColor','outlineColor'],
				names:{
			...
			solidColor:function(e,a){
				var
					r;
				if($.isNumber(a))a=_.colors.properties[a];
				do{
					r=$.curCSS(e,a);
			...
			return _.msie(7,1)?this.each(function(i,o){
				...
				if(b||d){
					if(!e.is("img")||o.complete){
						r=b?h:k.split(/[\(\)\"\']+/)[1];
						if(d)e.css(a,"none");
						else e.resize(e).attr("src",_.blankImage);
						e.msFilter({
							AlphaImageLoader:{
								src:r,
								sizingMethod:'crop'
	...
	$.each(_.colors.properties,function(i,v){
